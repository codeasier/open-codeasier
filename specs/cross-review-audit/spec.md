...
| `run.evidence_contract`           | no `judgeModel` ⇒ persisted nonblank `context`, successful adapter snapshot, or copied parent-pack snapshot; `judgeModel` and no shared evidence ⇒ `gatherer` object exists (any gatherer status). `insufficient-evidence` when `adapterGatherer.status` is `failed` and no reviewer exists |
| `adapter.gather`                  | no `adapterGatherer`, or `adapterGatherer.status === "succeeded"`; `fail` carries the bounded adapter error                                                                                                                                                                                 |
...
12. **Failed PR adapter.** `gh` or gitcode-cli failed; manifest is `phase: failed`, `reviewers: []`, `adapterGatherer.status: failed`. `adapter.gather` fails with the adapter error, `run.evidence_contract` is `insufficient-evidence`, and the skill does not grade P2.
