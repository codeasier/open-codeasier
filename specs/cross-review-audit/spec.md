...
14. Do not fetch a session ID that is not the parent or a manifest role ID. Isolation is inferred from: role `parentID` equals the owner; prompt `tools.task === false`; no successful `task` tool part; for snapshot runs, the SDK session `directory` equals `snapshot.worktree`. If the SDK cannot list children of a role session, check `role.no_children` is `insufficient-evidence` unless a successful `task` part is present (then `fail`).
15. Parent evidence keeps every `cross_review`, `cross_review_config`, `cross_review_start`, `cross_review_status`, `cross_review_cancel`, and `cross_review_finalize` tool part (input + status). Other parent messages follow existing byte/message caps. Child evidence keeps the linked user prompt and tool parts (including `invalid` / denied `bash`).
    15a. Role sessions are read with `query.directory` set to the snapshot worktree first, then the run directory, then the caller directory; a `SESSION_NOT_FOUND` or `SDK_FAILURE` answer moves to the next directory because a finalized or cancelled run may already have removed its worktree. The directory-binding check uses the SDK session record, never the query directory that succeeded, and compares both sides in the same `realpath` canonical form the runtime uses for `assertSessionDirectoryBinding` (falling back to the resolved path when a directory no longer exists).
...
| `run.evidence_contract`           | no `judgeModel` ⇒ persisted nonblank `context`, successful adapter snapshot, or copied parent-pack snapshot; `judgeModel` and no shared evidence ⇒ `gatherer` object exists (any gatherer status). `insufficient-evidence` when no reviewer exists and either `adapterGatherer.status` is `failed` or a PR snapshot persist omitted `adapterGatherer` (`snapshot.forge` set, `source` is not `parent-pack`) |
| `adapter.gather`                  | no `adapterGatherer` (except the omitted-legacy PR persist above, which is `insufficient-evidence`), or `adapterGatherer.status === "succeeded"`; `fail` carries the bounded adapter error                                                                                                                                                                                                                  |
...
| `role.session.directory`          | no snapshot, or the SDK session `directory` resolves to `snapshot.worktree`; unreadable session or missing `directory` ⇒ `insufficient-evidence`                                                                                                                                                                                                                                                            |
...
| `role.prompt.tools_deny`          | no `READ_ONLY_TOOLS` key is present with a value other than `false`, and every write key (`READ_ONLY_TOOLS` minus `PRIMARY_TOOL_IDS`) is present; absent primary-session-gated keys pass with a detail that names them                                                                                                                                                                                      |
...
Empty `context` is omitted, matching start/gather rules. Since evidence packs, a non-PR start with `context` persists a `parent-pack` snapshot instead of `context`; `hasContext` therefore marks only legacy embed runs.

### Behavioral evidence (tool emits, skill judges)

18. Per run, emit a compact parent protocol timeline attributed by `runID` from start outputs and status/cancel/finalize args. Unattributed calls (including `cross_review_config` previews) remain on every run. Each call includes time, selected args (`target` truncated, `context` length or omitted, `hasEvidenceDir`, `reviewModelCount`, `judgeModel`, `agents`, `maxConcurrency`, `timeoutAction`, `detail`, `includeOutputs`, `waitMs`, `runID` when present), result `phase` / `status` / `runID` / `readyToFinalize` / `actionRequired` roles / bounded `warning`, and the bounded host `error` for rejected calls — not full status payloads. The emitted list is capped (first/last); omitted entries are counted and must not be invented. `run.legacy_tool.absent` stays parent-scoped over the unbounded call list.
    18a. Per run, emit `adapterGatherer` (forge, status, timestamps, bounded error), `snapshot` (worktree, snapshotDir, forge, source, evidenceDir, notes, url, SHAs), `finalStatus` from `finalResult.status`, and per role the manifest `startedAt`, `completedAt`, `deadlineAt`, `timeoutDetectedAt`, `timeoutExtensions`, `retry`, and bounded `error`. Role `output` is never emitted.
...
- **P1** model mismatch, tools-deny miss (a write tool absent or any deny key enabled), `role.session.directory` or parent-link break, reviewer children, silent model replace, legacy blocking tool used as the review path.
- **P2** evidence contract broken; reviewer wander/bash/search loop without shared evidence; timeout_pending resolved without a user `timeoutAction`; gatherer/judge role bleed; PR target started with a large `contextLength`.
- **P3** polling waste (repeated finalize while non-terminal, client-side sleep instead of `waitMs`), extra starts that did not replace a cancelled or failed run, missing `cross_review_config` preview.

`adapter.gather` fail is an environment failure reported in the run list, not a protocol break. A gatherer that failed, timed out, or was cancelled degrades the run to independent fetching by design. A `cross_review_cancel` after `completed` is snapshot cleanup.
...
12. **Failed PR adapter.** `gh` or gitcode-cli failed; manifest is `phase: failed`, `reviewers: []`, `adapterGatherer.status: failed`. `adapter.gather` fails with the adapter error, `run.evidence_contract` is `insufficient-evidence`, and the skill does not grade P2. The still-registered blocking `cross_review` persist omits `adapterGatherer` and `snapshot.source`; that shape is the same pair of `insufficient-evidence` results, and the adapter error is only in the parent tool-part.
13. **Removed snapshot worktree.** A completed run whose worktree was cleaned up; role sessions still resolve through the run or caller directory, and `role.session.directory` is graded from the session record.
14. **Older deny map.** Reviewer prompts dispatched before `cross_review_config` / `cross_review_audit` joined `READ_ONLY_TOOLS`; `role.prompt.tools_deny` passes and names the absent gated keys.
15. **Timeout decision.** A status result with `actionRequired` followed by one status with `timeoutAction`; the reviewer carries `timeoutDetectedAt` and `timeoutExtensions`. A `timeoutAction` whose result carries the stray warning is graded as pre-sent.
