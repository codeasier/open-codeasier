...
- `--reviewer-timeout-ms` accepts 5000-3600000 and defaults to 600000; the config `reviewerTimeoutMs` key accepts the same range. A timeout is enforced by the next status or finalize reconciliation, which aborts the overdue child and allows queued work and quorum evaluation to proceed.
