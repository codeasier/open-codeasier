...
{{ASK_REQUIRED_FIELDS}} only after presenting the list and recommendations. If the destination already exists at the selected scope, read it first and detect its reviewer format: a `reviewers` array or a flat `reviewModels` list. Ask the user to accept or modify the exact reviewer and judge identifiers, then collect max concurrency. Collect an optional reviewer timeout with the question tool: offer a menu of durations (for example 5, 10, 20, 30, 60 minutes) plus a no-preference option, and ask for a custom integer between 5000 and 3600000 milliseconds only if the user chooses one; leave the key out when the user has no preference. For a new destination or an existing `reviewers` destination, collect an optional focus for each reviewer; for an existing `reviewModels` destination, collect the flat model list plus one optional shared `focus` instead. Use `.opencode/cross-review.json` for local scope and `~/.config/opencode/cross-review.json` for global scope. Do not write until the user confirms the complete selection.

If the destination exists, require explicit confirmation before changing it and preserve its detected `reviewers` or `reviewModels` format; never write `reviewers` alongside an existing `reviewModels` (or vice versa), and on an explicitly confirmed format change replace the old key. If it does not exist, run `npx open-codeasier init` with the selected scope and optional path after confirmation; this creates an empty, model-free configuration. Write only the user's confirmed exact identifiers and any confirmed optional `reviewerTimeoutMs`, omit `judgeModel` when the parent session should judge, validate the completed JSON, and report the path plus the short `/cross-review <target>` invocation.
